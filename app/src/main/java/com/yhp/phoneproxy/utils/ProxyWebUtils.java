...
public class ProxyWebUtils {
    ...
    public static boolean setProxy(WebView webview, String host, int port) {
        ...
        if (sdkInt < Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            return setProxyPreICS(webview.getContext(), host, port);
            ...
            return setProxyKK(webview.getContext(), host, port, "android.app.Application");
        } else {
    ...
    public static boolean clearProxy(WebView webView) {
        int sdkInt = Build.VERSION.SDK_INT;
        if (sdkInt < Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
            return setProxyPreICS(webView.getContext(), null, 0);
        } else if (sdkInt < Build.VERSION_CODES.JELLY_BEAN) {
            return setProxyICS(webView, null, 0);
        } else if (sdkInt < Build.VERSION_CODES.KITKAT) {
            return setProxyJB(webView, null, 0);
        } else if (sdkInt < Build.VERSION_CODES.LOLLIPOP) {
            return setProxyKK(webView.getContext(), null, 0, "android.app.Application");
        } else {
    ...
    private static boolean setProxyPreICS(Context context, String host, final int port) {
        ...
        try {
            ...
            network = getInstanceMethod.invoke(networkClass, new Object[]{context});
        } catch (Exception ex) {
    ...
    @SuppressLint("NewApi")
    @SuppressWarnings("all")
    private static boolean setProxyKK(Context appContext, String host, int port, String applicationClassName) {
        if (null == host) {
